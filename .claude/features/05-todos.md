...
**Three real bugs surfaced, two while writing tests and one reported by the user after Phase 2 shipped — all fixed, and all three are retroactive fixes to Phase 1:**
- **The checkbox had no visible border when unchecked.** `AnimatedCheckbox` used a `border-1.5` class, which isn't a real Tailwind utility (no fractional border-widths are generated; confirmed by grepping the built CSS for it — nothing matched). With no border-width or border-style applied, an unchecked box (transparent background, no border) was invisible against the page. **Reported by the user after Phase 2 shipped.** Fixed by switching to the plain `border` utility (1px, the project's standing hairline convention), which does generate real CSS. No test caught this, because none of the tests assert on computed styles or take a screenshot — testing-library confirms an element exists and responds to events, not that it's visually visible. Worth remembering: a class that silently fails to match Tailwind's generated utilities (a typo, or an assumed-valid fractional value) produces no error anywhere, just missing CSS.
