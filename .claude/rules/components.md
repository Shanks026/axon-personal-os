...
- **Multi-value fields in a dialog go in the body, not inside their picker's trigger** (2026-09-25). The task dialog lists its selected tags as removable pills under the description (the Tags chip stays "Tags · n"). Its links are an inline, borderless "Add a link" input with a visible + button, styled like the description, with the saved links listed above it. There's no popover.
- **Hover-to-open property chips** (the task dialog's status, priority, date and tags chips, 2026-09-25): a `useHoverOpen()` hook (`src/hooks/`) opens the trigger's Popover/DropdownMenu on hover as well as on click, so a pointer user can preview and change a property with fewer clicks. Pass its `hoverProps` to both the trigger and the content (so moving the pointer between them doesn't flicker-close), and **a hover-driven DropdownMenu must be `modal={false}`**. A modal menu blocks pointer events on its own trigger, which fires mouseleave → close → mouseenter → open in an endless flicker. Popovers are non-modal by default, so they don't have this problem. Only opt a component into this via a `hoverOpen` prop — row, card and board menus stay click-only, so scanning a list doesn't pop menus open unexpectedly.
