...
| Right rail | **352px (`w-88`)** for task meta and the note side panel (the user's choice, 2026-09-26; matching the 256px sidebar was too small). It becomes a Sheet below 1024px. |
...
- **Task detail page** (Feature 07): a main column capped at 720px (`max-w-180`), padded 40/56.
  ...
  - **Rail:** **352px** (`w-88`), the same as the note rail (the user's choice, 2026-09-26, after trying 300, 320 and a sidebar-matching 256, which was too small) with 84px labels (`w-21`), and a Sheet below `lg`. Link cards are 36px bordered rows (an icon, "!1431 · group/project", ↗). The footer has a Pinned switch and a `destructive` "Move to Trash".
