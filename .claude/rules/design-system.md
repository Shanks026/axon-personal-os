...
- **Task detail page** (Feature 07): a main column capped at 720px (`max-w-180`), padded 40/56.
  ...
  - **Rail:** **304px** (`w-76`), the same as the note rail (set by the user, 2026-09-26, after 256, 352 and 320) with 84px labels (`w-21`), and a Sheet below `lg`. Link cards are 36px bordered rows (an icon, "!1431 · group/project", ↗). The footer has a Pinned switch and a `destructive` "Move to Trash".
- **Tags and Versions rows in the detail rails** (task and note, the user's request, 2026-09-26): the label on its own line and the badges wrapping below (side by side looked cramped). The dashed "+ Tag" / "+ Version" add buttons match the badges they add: `h-5`, `rounded-sm`, `text-xs` (not full pills).
...
- **Calendar** (Feature 08, design Calendar + Overlays → event): the page title row is the range title (`text-3xl`) with a mono fiscal subtitle ("Q2 FY 2026–27", "· W13" outside Month, "· Global" in Global), then Today, ‹ › (tooltips with `Kbd`), the view `SegmentedControl`, a `⋯` layers menu (Tasks due / Todos due) and "New event". The month grid is always 6 rows in a `rounded-xl` border, hairlines between cells, weekends `bg-muted/40`, today a filled destructive circle. Chips are 20px, `text-xs`: a filled dot (event) or hollow square (due task/todo; a todo's square is its checkbox, done fills it with a check) in the item's space colour via `dotClasses`/`textClasses` (`ItemMarker`), a mono faint start time, the title. Up to 3 per day, else 2 + "+N more" (popover). Agenda: sticky day headers ("Today" in destructive), 44px rows with a 112px mono time column. **Week/Day grid** (Phase 2): a 56px (`w-14`) mono faint hour gutter and 56px hours (`h-14` rows, a 1344px column), weekend columns `bg-muted/40`, and a sticky header whose cells hold the weekday, the day number (today destructive, semibold) and the all-day chips. Event blocks (`eventBlockClasses` in `lib/tint.js`): `rounded-md`, a 3px left border in the space colour with the palette's 50 fill in light (100 for slate; lightened at the user's request, 2026-09-26) and a 15% wash in dark, the title `text-xs font-medium` and the time mono muted; under 30 minutes, the title only. The now-line is a 2px destructive line with an 8px dot. A dragged block leaves a dashed `border-border-strong` ghost; the overlay copy is `bg-card` with `shadow-md` at scale 1.02; the resize bar is 18×3px in the space colour, shown on hover. The slot-selection ghost is dashed on `bg-accent/60` with the mono range. The event dialog is read-style: a large title, then 38px icon rows (clock, map-pin, video, align-left, square-check-big, the space emoji) with borderless inputs that hover to `bg-accent`. **The time row uses the normal sans font** for its dates and times, not mono (the user's request, 2026-09-26): date buttons and time inputs are both 32px tall with `text-sm`, and the All day switch sits on its own line below them. Each row's icon sits in a 38px box at the top, so it lines up with the first line when a row wraps. Its footer (edit mode): the meeting-note outline button ("Create meeting note" with `file-plus`, or "Open meeting note" with `notebook-pen`), a `destructive` icon Delete with a tooltip, a spacer, Cancel and Save. Events with a meeting note show a 12px muted `notebook-pen` after the title. The note rail's **Meeting** card (above Linked tasks) is a `rounded-lg` bordered block: calendar icon and title, the mono date and time, the location, and a small "Open in calendar ↗" link.
