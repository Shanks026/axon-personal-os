...
- **Note card** (Feature 06, following the grid task card): `bg-card`, `rounded-xl`, at least 148px tall (`min-h-37`), the whole card a link. A 2-line semibold title ("Untitled" in faint) with ⋮ (Pin/Unpin, Move to Trash) on its right, a 2-line muted excerpt, a spacer, tags pinned above the footer (`TagPillGroup max={3}`), and a dashed footer with "Updated 2d ago" in the normal sans font (space emoji before it in Global). The notes list has **no sort** (always newest edit first) and shows a **Pinned** section before **All notes**; its second view is a Table (Note + excerpt, Space in Global, Tags, Updated, ⋮).
- **Note editor** (Feature 06): the sidebar collapses to its rail on `notes/:noteId` (temporarily; the saved preference returns on leave). The header holds the save state, a pin toggle, a Details (`panel-right`) toggle and ⋮. A 680px reading column (`max-w-170`): the display title, then the tags row (pills, a dashed "+ Tag" pill, mono "Edited 2h ago"), then the body in `text-base leading-7`. A 280px right rail (`w-70`, hairline left border) lists Space, Created, Updated and Words; below `lg` the toggle opens a Sheet instead.
