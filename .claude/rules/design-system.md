...
- **Task detail page** (Feature 07): a main column capped at 720px (`max-w-180`), padded 40/56.
  ...
  - **Rail:** **352px** (`w-88`; widened twice at the user's request, from the design's 300, the same width as the note rail) with 84px labels (`w-21`), and a Sheet below `lg`. Link cards are 36px bordered rows (an icon, "!1431 · group/project", ↗). The footer has a Pinned switch and a `destructive` "Move to Trash".
