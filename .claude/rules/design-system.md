...
| Right rail | **320px (`w-80`)** for task meta and the note side panel (the user's final choice, 2026-09-26; 256 was too small, 352 too wide). It becomes a Sheet below 1024px. |
...
- **Task detail page** (Feature 07): a main column capped at 720px (`max-w-180`), padded 40/56.
  ...
  - **Rail:** **320px** (`w-80`), the same as the note rail (the user's final choice, 2026-09-26: 256 matching the sidebar was too small, 352 too wide) with 84px labels (`w-21`), and a Sheet below `lg`. Link cards are 36px bordered rows (an icon, "!1431 · group/project", ↗). The footer has a Pinned switch and a `destructive` "Move to Trash".
